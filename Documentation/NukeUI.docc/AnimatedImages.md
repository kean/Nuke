...
What it buys is memory and not time. Image I/O inflates the whole canvas whatever size is asked of it and resamples afterwards, so a frame decoded small costs what the full-size one costs – a little more, in fact, since the resampling is work a full-size decode never does. The saving is all in the bytes, and the bytes are the whole point: they are what decides whether the pool holds the animation whole and decodes it once, or plays it out of a window and decodes every frame again on every loop.
...
Views of the same animation at *different* sizes share too, as long as what is already decoded covers them and isn't far larger: a view draws from the smallest set of frames that covers it and scales them as it draws them, so the same sticker drawn a little smaller in one place than another is decoded once. A set answers a view only up to twice the longest side it asked for – four times the pixels – because the smaller view pays for those frames in bytes and goes on holding them after the larger view has gone: a 44-point avatar beside a full-screen animation decodes a set of its own rather than keeping 1000-pixel frames alive for it. Only a view that wants more pixels than anything decoded so far starts a set of its own; the sets already on screen stay where they are. ``AnimatedImageView`` also rounds the size it decodes for up to a step, so cells a fraction of a point apart ask for one size to begin with. The step is a small one because a larger one has nothing to offer: no size decodes faster than another, so rounding a view up to the next power of two would only ask for pixels nobody draws – across a spread of view sizes, twice the memory for frames that are no cheaper to produce and, minified again as the view draws them, no sharper than frames decoded at the size it needs.

The frames outlive the players holding them: a cell that scrolls off screen and comes back finds them still in memory. They are given back when the pool needs the room, or at the first sign of memory pressure, and go for good when the animation itself does – they last exactly as long as something, usually ``ImageCache``, still holds the ``AnimatedImageSource`` they came from.
...
Frames are decoded at the default priority, the frame a playhead is sitting on and the read-ahead behind it alike: a step between the two separates nothing the scheduler acts on, and above the default a wall of animations competes with the main thread that has to draw it. The decoder is paced by playback – one frame per frame shown – rather than by how much memory there is, which is what keeps a wall of animations from turning into CPU-bound work.

Each player is driven by a display link of its own, which runs while the player is playing and asks for no more than its animation needs – a 10 fps GIF asks for 20 Hz rather than the 120 the display is capable of. The system folds every link in the process into one wakeup per refresh, so a screen of them costs what one does. On AppKit a link is asked of the view being drawn into, so ``AnimatedImageView`` gets one and a player you build for a view of your own runs on a timer at the animation's own rate instead – as every player does on watchOS, and on macOS before 14.
