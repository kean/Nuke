...
The frame a playhead is sitting on is decoded at the priority of the screen – nothing is on screen until it lands – and the two frames of read-ahead behind it a step below, since they are due a frame or two from now and a wall of animations is decoding mostly those. The decoder is paced by playback – one frame per frame shown – rather than by how much memory there is, which is what keeps a wall of animations from turning into CPU-bound work.
