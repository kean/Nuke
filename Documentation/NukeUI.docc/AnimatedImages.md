...
Views of the same animation at *different* sizes share too, as long as what is already decoded is large enough: a view draws from the smallest set of frames that covers it and scales them as it draws them, so the sticker in a 44-point avatar draws from the frames of the same sticker in a 300-point bubble instead of decoding a second set. Only a view that wants more pixels than anything decoded so far starts a set of its own; the sets already on screen stay where they are, and the smaller view goes on holding the larger one's frames after that view has gone. ``AnimatedImageView`` also rounds the size it decodes for up to a step, so cells a fraction of a point apart ask for one size to begin with.
