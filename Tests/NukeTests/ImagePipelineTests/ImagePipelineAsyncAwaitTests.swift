...
struct ImagePipelineAsyncAwaitTests {
    ...
    @Test func imageRequestWithImageClosureSkippingDataLoadingQueue() async throws {
    ...
    @Test func imageRequestWithImageClosureSkippingDataLoadingQueueIsCancellable() async throws {
        // GIVEN a fetch closure that doesn't finish on its own
        let entered = TestExpectation()
        let proceed = AsyncGate()
        let request = ImageRequest(
