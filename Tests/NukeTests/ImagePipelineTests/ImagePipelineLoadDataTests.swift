...
struct ImagePipelineLoadDataTests {
    ...
    // The loader calls back on its own thread, and the first task to reach
    // the pipeline's actor applies every callback queued so far. It must not
    // reorder the chunks, let the completion overtake them, or apply them to
    // a cancelled task.
    ...
    @Test @ImagePipelineActor func failureIsReportedAfterTheChunksBeforeIt() async throws {
        ...
        // actor, so the pipeline applies them all at once
        let completed = TestExpectation(notification: ImagePipelineObserver.didCompleteTask, object: observer)
    ...
    @Test @ImagePipelineActor func cancellingWhileApplyingQueuedChunks() async throws {
        ...
        // the pipeline applies it all at once
        ...
        // from the rest of the queued callbacks
        await #expect(throws: ImagePipeline.Error.cancelled) {
            try await task.response
        }
