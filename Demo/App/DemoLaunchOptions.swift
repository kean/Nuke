...
struct DemoLaunchOptions {
    ...
    /// as the screen opens, once per launch: Concurrency Inspector starts a
    /// burst, Scroll Stress scrolls, Cancellation Torture and Cache Torture
    /// run their checks, Memory Soak runs for a minute, Animation Lab starts
    /// its soak. A script can then take a screenshot of the results without a
    /// tap.
...
extension DemoLaunchOptions {
    ...
    enum Argument: String, CaseIterable, Identifiable {
        ...
        var summary: LocalizedStringKey {
            ...
            case .autorun: "`1` starts the run of a Lab screen as soon as it opens, once per launch: Concurrency Inspector starts a burst of 240 requests, Scroll Stress scrolls for 10 s, Cancellation Torture runs its checks and the slot check, in about 15 s, Memory Soak runs for a minute, Cache Torture runs its checks, in about 12 s, and Animation Lab starts its hour-long soak. `0`, the default, waits for a tap."
            }
        }
    }
}
