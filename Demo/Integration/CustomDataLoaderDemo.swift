...
struct CustomDataLoaderDemo: View {
    ...
    private static let info = DemoInfo(
        ...
        points: [
            ...
            .init("The response", "Progress is measured against its `expectedContentLength`. A cancelled download resumes only if its response is an `HTTPURLResponse` with `Accept-Ranges` and a validator. The bundle loader's plain `URLResponse` has neither, so its loads start over."),
