...
public extension Request {
    /// Resizes the loaded image to *fill* (or *fit* depending on the
    /// `contentMode`) the given target size.
    /// 
    ...
    /// - parameter mode: An option for how to resize the image
    ...
    #if !os(watchOS)
    /// Resizes the loaded image to *fill* (or *fit* depending on the
    /// `contentMode`) the given target. Uses the current size of the target.
    ///
    /// This method replaces the current request's processor. If you want to add
    /// more processors to the request add them after calling this method.
    ///
